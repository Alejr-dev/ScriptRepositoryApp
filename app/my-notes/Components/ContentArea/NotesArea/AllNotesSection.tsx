...
function SingleNote({ note }: { note: SingleNoteType }) {
  ...
  return (
    <div
      ...
      max-sm:w-full rounded-md py-4   hover:translate-y-[-1px] ${selectedNote?._id === _id && !selectedNote.isTrash ? "border border-[#0083bb]" : ""} `}
    >
      <NoteHeader
        id={_id}
        title={title}
  ...
  return (
    <div className="flex  justify-between   items-center     mx-4 ">
      <span
        ...
        className={`font-bold text-lg w-[90%] cursor-pointer text-[#7a8b90] hover:text-[#0083bb]`}>
      ...
      {!isTrashed && (
        <Checkbox
          ...
          checkedIcon={
            <FavoriteIcon className="text-[#0083bb] cursor-pointer" />
          }
          checked={isFavorite}
  ...
  return (
    <div className="text-slate-500 text-[11px] mx-4 flex-wrap flex    gap-1 mt-4 ">
      {tags.map((tag, index) => (
        <span
          key={index}
          className="bg-purple-100 text-[#0083bb] p-1 rounded-md px-2"
        >
          {tag.name}
        </span>
      ))}
    ...
    try {
      ...
      toast((t) => (
        <div className={`flex gap-2 items-center`}>
          ...
          <button
            className="bg-[#0083bb] p-[4px] px-3 text-sm text-white rounded-md flex gap-1 items-center"
            onClick={() => {
              toast.dismiss(t.id);
    ...
    <div className=" flex justify-between   text-[13px] text-slate-400 mx-4 mt-3">
      ...
      <div className="flex gap-2 items-center">
        ...
        <DeleteRoundedIcon
          ...
          sx={{ fontSize: 17 }}
          className={`cursor-pointer ${note.isTrash && "text-[#0083bb]"} ${openContentNote ? "hidden" : ""}`}
        />
      </div>
    </div>
  );
